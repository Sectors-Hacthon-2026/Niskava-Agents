...
def test_format_investigative_prompt_is_english(temp_memory):
    """Regression: format_investigative_prompt() must produce English XML for LLM injection.
    Bug #2B — Indonesian text violates the English protocol rule in react_agent.py system prompt.
    """
    temp_memory.store_observation(
        source_label="User",
        source_type="USER",
        ...
        target_label="ANTM",
        target_type="TICKER",
        context_snippet="Autonomous investigation session TEST-001",
        session_id="TEST-001",
    )

    xml_block = temp_memory.format_investigative_prompt("ANTM", radius=2)

    assert "<investigative_memory>" in xml_block
    assert "Local graph memory recall" in xml_block

    for marker in ["lalu", "hari ini", "Rekam memori", "graf lokal untuk"]:
        assert marker not in xml_block, (
            f"English protocol violation: Indonesian '{marker}' in format_investigative_prompt"
        )


def test_record_investigation_context_snippets_are_english(temp_memory):
    """Regression: context_snippet strings auto-generated by record_investigation must be English.
    Bug #2C — hardcoded Indonesian f-strings in record_investigation().
    """
    class MockAnomaly:
        z_score = 2.8
        metric_type = "VOLUME"
        date = "2026-09-20"
        description = "Volume spike 2.8 sigma above MA20"

    class MockAnomalyNoDesc:
        z_score = 3.1
        metric_type = "PRICE"
        date = "2026-09-21"
        description = ""

    temp_memory.record_investigation(
        session_id="INV-EN-001",
        ticker="BBRI",
        anomalies=[MockAnomaly(), MockAnomalyNoDesc()],
        sector="Financials",
    )

    import sqlite3
    with sqlite3.connect(temp_memory.db_path) as conn:
        rows = conn.execute("SELECT context_snippet FROM memory_edges").fetchall()
        for (snippet,) in rows:
            if not snippet:
                continue
            for marker in ["Investigasi otonom", "Sektor industri", "Lonjakan", "sebesar", "sigma pada"]:
                assert marker not in snippet, (
                    f"English protocol violation: Indonesian '{marker}' in context_snippet: '{snippet}'"
                )
